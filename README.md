...
Built to demonstrate depth in real-time stream processing — not just batch ETL with a "streaming" label.
...
| dbt project (skeleton + config) | Done |
| dbt models (staging, core, marts) | In Progress |
| GTFS Static dimension tables + seed data | Planned |
| Airflow DAG (weekly dim refresh) | Planned |
...
- **Local dev with DuckDB** — `dbt run` completes in 1-2s locally; cloud deployment uses `dbt-databricks`.

## Tech Stack

| Component | Technology | Purpose |
|-----------|-----------|---------|
| Ingestion | Python + `confluent-kafka` | Poll AT APIs, serialize to Avro, publish to Kafka |
| Message Broker | Azure Event Hubs (prod) / Redpanda (local) | Kafka-compatible streaming backbone |
| Schema Registry | Confluent-compatible (Redpanda bundled) | Avro schema evolution + validation |
| Stream Processing | PySpark Structured Streaming | Bronze ingestion + Q1-Q4 detection jobs |
| Batch Transform | dbt (`dbt-duckdb` local, `dbt-databricks` prod) | Silver/Gold layer, historical analytics |
| Orchestration | Apache Airflow | Weekly GTFS static refresh + dbt trigger |
| Storage | Delta Lake (prod) / Parquet (local) | Medallion architecture |
...
├── transform/                      # dbt project (WIP — skeleton + config done, models in progress)
│   ├── models/
...
uv run python src/ingestion/at_producer.py
```

### Run Streaming Jobs

Each job runs as a separate long-lived process:

```bash
# Bronze ingestion (raw events → Parquet/Delta)
uv run python src/streaming/bronze_ingestion.py

# Q1-Q4 detection jobs
uv run python src/streaming/delay_alert_job.py
uv run python src/streaming/vehicle_stall_job.py
uv run python src/streaming/headway_regularity_job.py
uv run python src/streaming/alert_correlation_job.py
```

All jobs support graceful shutdown via `Ctrl+C` (SIGINT/SIGTERM).
...
uv run dbt run       # build staging → core → marts
...
**Bronze**: Raw event tables (VP, TU, SA) + detection output tables (Q1-Q4). Written by Spark Streaming.

**Silver**: Cleaned views over Bronze. Joins with dimension tables for route names. No business logic flags.
